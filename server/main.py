...
app = FastAPI(title="HA Grok Bridge 0.9.9", version=APP_VERSION)
...
def new_id() -> str:
    return f"ha-099-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{secrets.token_hex(4)}"

async def execute(command: str, allow_mutation: bool, timeout: int) -> dict[str, Any]:
    command = command.strip()
    if not command_allowed(command):
        raise HTTPException(403, "command is not allowed by the 0.9.9 server policy")
