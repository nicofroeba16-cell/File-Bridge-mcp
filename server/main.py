...
async def health() -> dict[str, Any]:
    return {
        "ok": True,
        "version": APP_VERSION,
        "repository": transport.repo,
        "branch": transport.branch,
        "github_configured": bool(transport.token),
...
async def capabilities() -> dict[str, Any]:
    return {
        "version": APP_VERSION,
        "mode": "read-only-default",
        "transport": "github-command-result",
        "tools": tool_list()["tools"],
        "mutations": "explicit-confirmation-required",
    }
...
async def ha_list(req: FileRequest) -> dict[str, Any]:
    path = safe_config_path(req.path)
    command = f"git -C /config ls-tree -r --name-only HEAD -- {path}"
    return await execute(command, False, req.timeout)
...
async def local_list(path: str = ".") -> dict[str, Any]:
    return {"files": workspace.list(path)}

async def local_search(query: str, path: str = ".", regex: bool = False, max_results: int = 100) -> dict[str, Any]:
    return {"matches": workspace.search(query, path, regex, max_results)}

async def local_analyze(path: str) -> dict[str, Any]:
    return {"analysis": workspace.analyze(path)}
...
def _validate_modern_headers(message: dict[str, Any], headers: dict[str, str]) -> str | None:
    ...
    if version != MCP_MODERN:
        return f"MCP-Protocol-Version must be {MCP_MODERN}"
    if method_header != message.get("method"):
        return "Mcp-Method does not match JSON-RPC method"
    method = message.get("method")
    params = message.get("params") or {}
    if method == "tools/call" and name_header != params.get("name"):
        return "Mcp-Name does not match tools/call name"
    return None


async def _dispatch_tool(name: str, args: dict[str, Any]) -> dict[str, Any]:
    if name == "ha_capabilities":
        return await capabilities()
    if name == "ha_status":
        return await ha_status(CommandRequest(command="ha info"))
    if name == "ha_read_file":
        if "path" not in args:
            raise ValueError("path is required")
        return await local_read(args["path"])
    if name == "ha_list_files":
        return await local_list(args.get("path", "."))
    if name == "ha_run_allowed_command":
        return await ha_run(CommandRequest(**args))
    if name == "ha_verify":
        return await ha_verify(VerifyRequest(**args))
    if name == "ha_search":
        return await local_search(args["query"], args.get("path", "."), args.get("regex", False), args.get("max_results", 100))
    if name == "ha_analyze":
        return await local_analyze(args["path"])
    if name == "ha_write_file":
        return await local_write(args["path"], args["content"], args.get("expected_sha256"), args.get("confirm", False))
    if name == "ha_patch_file":
        return await local_patch(args["path"], args["old"], args["new"], args.get("expected_sha256"), args.get("count", 1), args.get("confirm", False))
    if name == "ha_backup":
        return await local_backup(args.get("path", "."), args.get("confirm", False))
    if name == "ha_rollback":
        return await local_rollback(args["backup_id"], args.get("confirm", False))
    raise KeyError(name)
...
async def _mcp_dispatch(message: dict[str, Any], request_headers: dict[str, str] | None = None) -> dict[str, Any] | None:
    ...
    if request_headers.get("mcp-protocol-version") == MCP_MODERN:
        ...
        if header_error:
            return _mcp_error(request_id, -32020, header_error)
    if method == "initialize":
        return {"jsonrpc": "2.0", "id": request_id, "result": {
            "protocolVersion": MCP_LEGACY,
            "capabilities": {"tools": {"listChanged": False}},
            "serverInfo": {"name": "ha-grok-bridge", "version": APP_VERSION},
        }}
    ...
    if method == "tools/call":
        ...
        try:
            value = await _dispatch_tool(name, args)
        except KeyError:
            return _tool_error(request_id, f"unknown tool: {name}")
        except (TypeError, ValueError) as exc:
            return _tool_error(request_id, str(exc) or "invalid tool arguments")
        except HTTPException as exc:
            return _tool_error(request_id, str(exc.detail))
        except Exception as exc:
            return _tool_error(request_id, f"tool execution failed: {exc}")
        import json
        return {"jsonrpc": "2.0", "id": request_id, "result": {
            "content": [{"type": "text", "text": json.dumps(value, ensure_ascii=False, indent=2)}],
            "structuredContent": value,
            "isError": not bool(value.get("verified", True)),
        }}
