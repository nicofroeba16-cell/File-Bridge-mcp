...
def test_stdio_initialize_and_tools_list() -> None:
    ...
    assert lines[0]["result"]["serverInfo"]["version"] == "0.9.6-dev"
    names = {tool["name"] for tool in lines[1]["result"]["tools"]}
    assert {"ha_capabilities", "ha_read_file", "ha_write_file", "ha_patch_file"} <= names
    assert {"ha_control_read", "ha_control_write", "ha_control_browse", "ha_control_sync", "ha_bridge_status"} <= names
    assert len(names) == 17
    assert proc.stderr == ""
